...
using MadBug.WebAPI.Providers;
using MadBug.WebAPI.Results;
using MadBug.Data;
using MadBug.Data.Repositories;
using MagBug.Domain;

namespace MadBug.WebAPI.Controllers
{
    [Authorize]
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Register(RegisterBindingModel model)
        {
            ...
            using (var tx = appDbContext.Database.BeginTransaction()) {

                using (var txData = dataContext.Database.BeginTransaction()) {
                    ...
                    if (!result.Succeeded)
                    {
                        ...
                        try
                        {
                            var userDb = new User { Id = user.Id, CreatedAt = DateTime.Now, DisplayName = user.Email };
                            UserRepository userRepository = new UserRepository(dataContext);
                            userRepository.Insert(userDb);
                            dataContext.SaveChanges();
                            //Commit en ambos contextos
                            tx.Commit();
                            txData.Commit();
                            dataContext.Dispose();
                        } catch (Exception ex) {
                            //Si una operación falla se realiza rollback en ambos
                            tx.Rollback();
                            txData.Rollback();
